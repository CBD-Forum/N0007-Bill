        ...
        <div class="whcontent__info" >
          <el-form label-width="130px" class="demo-ruleForm">
            ...
            <el-form-item label="票据类型">
              ...
              <el-checkbox v-model="checkList.type" style="float:right" :disabled="checked.checked1"></el-checkbox>
            </el-form-item>
            <el-form-item label="票据编号">
              <span class="examineInfo">{{whDate.bill_number}}</span>
              <el-checkbox v-model="checkList.bill_number" style="float:right" :disabled="checked.checked2"></el-checkbox>
            </el-form-item>
            <el-form-item label="出票人">
              <span class="examineInfo">{{whDate.drawer}}</span>
              <el-checkbox v-model="checkList.drawer" style="float:right" :disabled="checked.checked3"></el-checkbox>
            </el-form-item>
            <el-form-item label="承兑人">
              <span class="examineInfo">{{whDate.acceptor}}</span>
              <el-checkbox v-model="checkList.acceptor" style="float:right" :disabled="checked.checked4"></el-checkbox>
            </el-form-item>
            <el-form-item label="收票人">
              <span class="examineInfo">{{whDate.taker}}</span>
              <el-checkbox v-model="checkList.taker" style="float:right" :disabled="checked.checked5"></el-checkbox>
            </el-form-item>
            <el-form-item label="承兑人类型">
              <span class="examineInfo">{{whDate.acceptor_type}}</span>
              <el-checkbox v-model="checkList.acceptor_type" style="float:right" :disabled="checked.checked6"></el-checkbox>
            </el-form-item>
            <el-form-item label="出票日" prop="name">
              <span class="examineInfo">{{whDate.issue_at}}</span>
              <el-checkbox v-model="checkList.issue_at" style="float:right" :disabled="checked.checked7"></el-checkbox>
            </el-form-item>
            <el-form-item label="票据到期日" prop="name">
              <span class="examineInfo">{{whDate.acceptance_at}}</span>
              <el-checkbox v-model="checkList.acceptance_at" style="float:right" :disabled="checked.checked8"></el-checkbox>
            </el-form-item>
            <el-form-item label="票据金额" prop="name">
              <span class="examineInfo">{{whDate.face_amount}}</span>
              <el-checkbox v-model="checkList.face_amount" style="float:right" :disabled="checked.checked9"></el-checkbox>
            </el-form-item>
...
export default {
  ...
  watch: {
    whopen: function(val) {
      setTimeout(()=>{
         if (val == true) {
          for (var checkd in this.checkList) {
            if(this.whDate[checkd+'_status']==1){
              console.log();
              this.checkList[checkd] = true;
            }else{
              this.checkList[checkd] = false;
            }
          }
  ...
  methods: {
    ...
    async examineSubmit(id) {
      ...
      this.whDate.face_amount=this.whDate.face_amount*100;
      this.whDate.possessor=this.whDate.possessor.toString();
      let result = api.billNoteSigns().signCreateNote(api.userkey(), this.whDate.possessor_address,this.whDate.possessor, api.transBillType(this.whDate.type), this.whDate.bill_number,this.whDate.drawer, this.whDate.taker, this.whDate.acceptor, Date.parse(new Date(this.whDate.acceptance_at)) / 1000, this.whDate.face_amount,'wwww');
