<template>
  <div class="trade">
    <div class="clearfix">
      <div class="title">
        <div class="clearfix">
          ...
          <div class="search-warp">
            ...
            <el-select class="search-warp__type" v-model="searchDate.type" placeholder="请选择">
              <el-option v-for="item in searchDateType" :label="item.label" :value="item.value">
              </el-option>
            </el-select>
            <el-date-picker class="search-warp__date" v-model="searchDate.startTime" type="date" :editable="false" placeholder="操作日-从" @change="changeStartTime" :picker-options="pickerOptions1">
            </el-date-picker>
            <el-date-picker class="search-warp__date" v-model="searchDate.endTime" type="date" :editable="false" placeholder="操作日-到" @change="changeEndTime" :picker-options="pickerOptions1">
            </el-date-picker>
      ...
      <div>
        <el-table :data="tradeRecord" style="width: 100%;margin-top:10px;" stripe :default-sort="{prop: 'date', order: 'descending'}">
          <el-table-column prop="created_at" label="操作时间" min-width="180">
          </el-table-column>
          <el-table-column prop="type" label="操作类型" min-width="120">
          </el-table-column>
          <el-table-column prop="bill_amount" label="票据金额(元)" min-width="120">
          </el-table-column>
          <el-table-column prop="acceptor" label="承兑人" min-width="120">
          </el-table-column>
          <el-table-column prop="acceptance_at" label="到期日" min-width="180">
          </el-table-column>
          <el-table-column prop="wechselspesen" label="贴现费" min-width="120">
          </el-table-column>
          <el-table-column prop="annualized_rate" label="贴现率%" min-width="120">
          </el-table-column>
          <el-table-column prop="opposite_name" label="交易对方" min-width="180">
          </el-table-column>
          <el-table-column prop="" label="区块链查询" min-width="120">
            <template scope="scope">
              <a v-if="tradeRecord[scope.$index].instruction!=null" :href="blockUrla+tradeRecord[scope.$index].instruction" target="_blank">查看</a>
            </template>
          </el-table-column>
         <!--  <el-table-column prop="user" label="转让申请人" min-width="180">
          </el-table-column>
          ...
          <el-table-column prop="transfer_amount" label="贴现金额（元）" sortable min-width="160" :formatter="formatter2">
          </el-table-column>
  ...
  data() {
    ...
    methods: {
      ...
      async getTradeRecord() {
        ...
        const data = await api.get(this, api.config.tradeRecord, this.searchDate);
        if (data.code == 200) {
          this.total = data.count;
          for(let x of data.data){
            x.opposite_name=x.opposite_name==null ? '--' :x.opposite_name;
      ...
      checkImg(index, rows) {
        ...
        var swiperData = [{
          ...
          title: "转让协议",
          src:rows.contract_file_path
        }];
        if (rows.contract_file_path == "") {
          swiperData.push({
            title: "协议信息",
            src: rows.contract_file_path
          })
      ...
      serachSubmit() {
        if (new Date(this.searchDate.startTime).getTime() > new Date(this.searchDate.endTime).getTime()) {
          ...
          //   this.searchDate.type = "已维护"
          // } else {
          //   this.searchDate.type = "已背书"
          // }
          this.getTradeRecord();
        }
      },
